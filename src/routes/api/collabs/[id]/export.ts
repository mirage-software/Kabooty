...
function buildassets(pick: any, zip: JSZip, process_counter: number) {
    ...
    assets.forEach((asset: any) => {
        const file = ServerPaths.generateAssetName(pick.id, asset.collabAssetId, asset.collabAsset.assetType) +
            '.png';
    ...
    if (pick.user != null) {
        ...
        if (pick != null) {
            ...
            if (pick.character != null) {
                ...
                series =
                    pick.character.anime_name == null
                        ? ''
                        : pick.character.anime_name;
            }
        }
